...
function restoreFormState() {
    try {
        ...
        if (Array.isArray(state.keywords)) {
            form.querySelectorAll('input[name="keywords"][type="checkbox"]').forEach((checkbox) => {
                checkbox.checked = state.keywords.includes(checkbox.value);
            });
        }
        
        // Restore custom keywords
        if (Array.isArray(state.custom_keywords) && state.custom_keywords.length > 0) {
            state.custom_keywords.forEach((keyword) => {
                addCustomKeyword(keyword, true);
            });
        }
    } catch (error) {
...
function renderCustomKeywordTags() {
    ...
    const customKeywords = getStoredCustomKeywords();
    container.innerHTML = customKeywords.map((keyword) => `
        <span class="badge bg-info text-dark d-inline-flex align-items-center gap-2">
            <span>${keyword}</span>
            <button type="button" class="btn-close btn-close-sm bi-custom-keyword-remove"
                    data-keyword="${keyword.replace(/"/g, '&quot;')}"
                    aria-label="Remove" style="font-size: 0.7rem;"></button>
        </span>
    `).join('');
}
 ...
 */
function addCustomKeyword(keyword, skipSave = false) {
...
function removeCustomKeyword(keyword, event) {
    ...
    const customKeywords = getStoredCustomKeywords();
    const filtered = customKeywords.filter((k) => k !== keyword);
    saveCustomKeywords(filtered);
    renderCustomKeywordTags();
    saveFormState();
}

function processAndStoreCustomKeywords(rawValue) {
    if (!rawValue) {
        return;
    }
    rawValue
        ...
        .filter(Boolean)
        .forEach((value) => addCustomKeyword(value));
}
...
function getTargetingParams() {
    ...
    // Only add keywords if there are any
                ...
                <div class="row g-2">
                    ...
                    <div class="col-12">
                        <div class="fw-semibold d-flex align-items-center gap-2">
                            ${entry.event_name}
                            ${entry.ai_analyzed ? '<span class="badge text-bg-primary" style="font-size:0.65rem;">AI</span>' : '<span class="badge text-bg-secondary" style="font-size:0.65rem;">…</span>'}
                        </div>
    ...
    const load = async () => {
        ...
        try {
            ...
            if (feedbackBlock) {
                ...
                feedbackBlock.textContent = labels.biEmptyEventsLabel;
            }
        } finally {
