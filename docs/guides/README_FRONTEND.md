...
- Any frontend modification must be documented in `docs/guides/README_FRONTEND.md`
...
- **Template integration:** `foodtrucks/templates/foodtrucks/dashboard.html`
- **Partial:** `foodtrucks/templates/foodtrucks/dashboard/partials/_bi_intelligence.html`
- **JS API helper:** `static/js/dashboard/api.js` (`fetchBusinessIntelligence`)
- **JS renderer:** `static/js/dashboard/dashboard.js` (`renderBusinessIntelligence`)

### Data source

- Owner endpoint: `/foodtrucks/<slug>/dashboard/bi/`
- Payload sections rendered in UI:
  - `best_spots`
  - `pricing_suggestions`
  - `event_opportunities`
  - `revenue_prediction`
...
- Read-only business intelligence (no auto-apply action in dashboard)
- Safe empty states per block
- Uses existing dashboard currency formatting
- Loaded in same refresh cycle as KPI/menu/slots panels