...
# 1️⃣0️⃣ IA décisionnelle (Business Intelligence)

## Règles

- L’IA ne doit jamais appliquer automatiquement une décision métier (prix, localisation, opportunité).
- Les suggestions IA doivent rester explicables et auditables.
- Le système doit continuer à fonctionner en mode déterministe si l’IA est indisponible.

## Implémentation requise

- Logique IA encapsulée dans des services dédiés (pas dans les modèles)
- Résultats structurés persistés en base (`LocationScore`, `PricingSuggestion`, `EventOpportunity`, `RevenuePrediction`)
- Fallback déterministe explicite dans chaque service
- Journalisation des sorties calculées (scoring / prédiction / suggestions)

## Interdictions

- Bloquer un flux propriétaire si une requête IA échoue
- Modifier automatiquement les tarifs menu à partir d’une suggestion
- Masquer la logique de décision sans `reason` / `breakdown`
